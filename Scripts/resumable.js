...
var Resumable = function(opts){
  ...
  $.appendBlob = function (blob) {
      $.files = [];
      var f = new ResumableFile($, blob);
      $.files.push(f);
  }
  ...
  function ResumableChunk(resumableObj, fileObj, offset, callback){
    ...
    $.test = function(){
      ...
      var url = ""
      var params = [];
      var query = (typeof $.resumableObj.opts.query == "function") ? $.resumableObj.opts.query($.fileObj, offset) : $.resumableObj.opts.query;
      $h.each(query, function(k,v){
          params.push([encodeURIComponent(k), encodeURIComponent(v)].join('='));
        });
      // Add extra data to identify chunk
        params.push(['resumableChunkNumber', encodeURIComponent($.offset+1)].join('='));
      //params.push(['resumableChunkNumber', encodeURIComponent($.resumableObj.hash)].join('='));
    ...
    $.send = function () {
      ...
      //formData.append('resumableChunkNumber', $.resumableObj.hash);
      ...
      //formData.append('resumableIdentifier', $.resumableObj.hash);
      ...
      var currentChunk = $.fileObj.file[func]($.startByte, $.endByte);
      ...
      //$.xhr.open("POST", '/sandbox');
        ...
        //DELL TEAM: Once current chunk is hashed POST the chunk with hash data
      hashFile(currentChunk).done(function (hash) {
          formData.append('resumableChunkHash', hash);
          $.xhr.send(formData);
        });      
    }
    $.abort = function(){
